...
#Grab final game from masterdelete table and add to the players table
UPDATE masterdelete
SET finalGame = 
	CASE 
		WHEN finalGame = '' THEN NULL
        ELSE finalGame
	END;
UPDATE players
SET finalGame =
	(SELECT masterdelete.finalGame
	FROM masterdelete
    WHERE masterdelete.playerID = players.playerID);


#Remove the nominees not inducted to hall of fame
DELETE FROM HallOfFame
WHERE inducted != 'Y';
#Remove players not in our players table for HOF
DELETE FROM HallOfFame
WHERE playerID NOT IN
	(SELECT playerID
    FROM players);
#Remove duplicate players from the table (added a temporary ID)
DELETE T1 FROM HallOfFame AS T1  
INNER JOIN HallOfFame AS T2   
WHERE T1.id > T2.id AND T1.playerID = T2.playerID;  
